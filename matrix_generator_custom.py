...
# filedir = r'C:/Users/gusev/Documents/workspace/IPCSP/data/'
filedir = r'../data/'
# pt.Lattice.cifLocation = filedir
...
class Phase:
    ...
    def __init__(self, phase_name):
        ...
        try:
            ...
            self.gar_param = {}
            self.gar_param[('Al', 'O')] = {'De': 0.361581, 'a0': 1.900442, 'r0': 2.164818, 'A': 0.9, 'lo': 0.0, 'hi': 15.0}
            self.gar_param[('Ca', 'O')] = {'De': 0.030211, 'a0': 2.2413340, 'r0': 2.923245, 'A': 5.0, 'lo': 0.0, 'hi': 15.0}
            self.gar_param[('O', 'O')] = {'De': 0.042395, 'a0': 1.379316, 'r0': 3.618701, 'A': 22.0, 'lo': 0.0, 'hi': 15.0}
...
@njit(cache=True)
def BuckinghamTwoIons(pos_i, pos_j, cell_size, A, rho, beta, lo, hi, closest_distance, vects):
    '''
            ...
            for j in range(-max_cells, max_cells+1):
                for k in range(-max_cells, max_cells+1):
                    if not (i == 0 and j == 0 and k == 0):
                        ...
                        j_ = j*vects[1][1]
                        k_ = k*vects[2][2]
                        r = cell_size*np.linalg.norm(pos_j + np.array([i_, 0, 0]) +
                            np.array([0, j_, 0]) + np.array([0, 0, k_]) - pos_i)
                        if r <= hi:
...
def _Buck_row(pos_i, grid_size, cell_size, A, rho, beta, lo, hi, closest_distance):
    ...
    # if pos_i == 58:
    #     print(BuckinghamTwoIons(positions[pos_i], positions[59], cell_size, A, rho, beta, lo, hi, closest_distance))
    for pos_j in range(pos_i, grid_size**3):
        # print(BuckinghamTwoIons(pos_i, pos_j, cell_size, A, rho, beta, lo, hi, radius_threshold))
        result[pos_j] = BuckinghamTwoIons(positions[pos_i], positions[pos_j], cell_size, A, rho, beta, lo, hi, closest_distance)
    return result
...
def _gar_row(pos_i, grid_size, cell_size, De, a0, r0, A, lo, hi, closest_distance):
    ...
    for pos_j in range(pos_i, grid_size**3):
        # def GarnetTwoIons(pos_i, pos_j, cell_size, De, a0, r0, A, lo, hi, closest_distance):
        result[pos_j] = GarnetTwoIons(positions[pos_i], positions[pos_j], cell_size, De, a0, r0, A, lo, hi, closest_distance)
    return result

# @jit(parallel=True)
def generate_Buck(grid_size, cell_size, phase, output_directory, radius_threshold=0.9, multicpu = False):
    '''
    ...
    for ion_pair, potential_param in phase.buck.items():
        ...
        if ion_pair in phase.closest_distance:
            ...
            print("Closest distance for " + ion_pair[0] + '-' + ion_pair[1] + " was set to be " + str(phase.closest_distance[ion_pair]))

        # print(closest_distance)
        # print(potential_param)
        # sleep(3)
        buck_row = partial(_Buck_row, grid_size=grid_size, cell_size=cell_size, A = potential_param['par'][0], rho=potential_param['par'][1], beta=potential_param['par'][2], 
    ...
    #     result[i] = test()
        ...
        filename = 'C{grid_size}_'.format(grid_size=grid_size)+ion_pair[0]+ion_pair[1]+'_{cell_size}.npy'.format(cell_size=cell_size)
...
def generate_garnet(grid_size, cell_size, phase, output_directory, radius_threshold=0.9, multicpu = False):
    '''
    HACK for the garnet
    '''
    for ion_pair, potential_param in phase.gar_param.items():
        ...
        # def GarnetTwoIons(pos_i, pos_j, cell_size, De, a0, r0, A, lo, hi, closest_distance):
        garnet_row = partial(_gar_row, grid_size=grid_size, cell_size=cell_size, De = potential_param['De'], a0=potential_param['a0'], r0=potential_param['r0'], 
            A=potential_param['A'], lo=potential_param['lo'], hi=potential_param['hi'], closest_distance=closest_distance)
...
def get_Ewald(grid_size, cell_size):
    '''
    TODO: invoke generator when the file is not available
    '''
    filename = 'C{grid_size}.npy'.format(grid_size=grid_size)

    with open(filedir+'/Ewald/'+filename, 'rb') as f:
        # return np.load(f)*(cell_size*1.0524791771928207974642457789917/grid_size)
...
if __name__ == "__main__":
    ...
    output_directory = '/home/sanantoniochili/Desktop/PhD/Scripts/IPCSP/Ewald'
    for size in size_range:
        generate_Ewald(size, output_directory)
