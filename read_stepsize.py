...
if __name__ == "__main__":
	...
	for filename in flist:
		with open(filename, 'r') as file:
			...
			for line in file:
				if "__s__" in line:
					step = line.split('...')[1].rstrip().split(' ')[-1]
					Steps.append(step)
				if "Cycle" in line:
		...
		count += 1
	for d in dirs:
		with open(args.test_dir+'/'+d+'/'+args.ofilename+'_energy.csv', 'w') as f:
			dfes.to_csv(f, header=True)

		with open(args.test_dir+'/'+d+'/'+args.ofilename+'_gnorm.csv', 'w') as f:
			dfgs.to_csv(f, header=True)

		with open(args.test_dir+'/'+d+'/'+args.ofilename+'_step.csv', 'w') as f:
